...
{
    internal class Euler0008 : Euler
    {
        public Euler0008() : base()
        {
        ...
        public override void Run()
        {
            ...
            // now go through each and find the product of the series
            const int lengthOfValues = 13;
            long greatestProduct = 0;
            int solutionPosition = 0;
            int firstPosition = 0;
            int lastPosition = numbersAsCharArray.Length - lengthOfValues;
            for (int i = firstPosition; i <= lastPosition; i++)
            {
                long currentProduct = 1;
                for (int j = 0; j < lengthOfValues; j++)
                {
                    int currentValue = numbersArray[i + j];
