...
let run () =
    ...
    let nodesFromInput (input:int[][]) start hFunction =
        ...
        if finished.Length > 0 && (finished[finished.Length - 1]).position = goalPosition 
            then finished
                ...
                let sortedQueue = queue |> Array.sortBy (fun n -> 
                    let hCost = match n.heuristicCost with
                        | None -> System.Int32.MaxValue
                        | Some(x) -> x
                    let dCost = match n.distanceTo with
                        | None -> System.Int32.MaxValue
                        | Some(x) -> x
                    hCost + dCost
                    )
    ...
    let nodes = nodesFromInput input start hFuction
